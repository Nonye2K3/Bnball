...
export function getContractAddress(chainId: number): `0x${string}` {
  switch (chainId) {
    ...
    default:
      throw new Error(
        `Unsupported chain ID: ${chainId}. Please switch to BSC Mainnet or Testnet.`,
      );
  }
}
...
export const PLATFORM_FEE_RECIPIENT =
  "0xC196dc762FbC2AB044AAEAc05E27CD10c4982a01" as `0x${string}`;
...
if (!isValidEthereumAddress(PLATFORM_FEE_RECIPIENT)) {
  console.error(
    `Invalid platform fee recipient address: ${PLATFORM_FEE_RECIPIENT}`,
  );
}
