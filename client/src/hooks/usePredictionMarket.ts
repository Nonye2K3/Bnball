  ...
  const placeBet = async (
    ...
    if (!contractMarketId || contractMarketId === 0) {
      toast({
        ...
        description:
          "This market has not been deployed on the blockchain yet. Only deployed markets accept bets.",
        variant: "destructive",
      });
      return;
    }
...
export function useCreateMarket() {
    ...
    const persistMarket = async () => {
      if (isSuccess && hash && address && marketDataRef.current) {
        try {
          ...
          const recordedRegistrationFee =
            marketDataRef.current.registrationFeeWei;
          const totalRecordedValue =
            BET_CONFIG.CREATE_MARKET_STAKE + recordedRegistrationFee;
          ...
          await persistTransaction({
            ...
            status: "success",
            chainId,
            value: totalRecordedValue.toString(),
            metadata: JSON.stringify({
              title: marketDataRef.current.title,
              category: marketDataRef.current.category,
              registrationFeeWei: recordedRegistrationFee.toString(),
            }),
          });
          ...
          toast({
            ...
            description:
              "Your market is live on-chain but failed to save locally. It will sync automatically.",
            variant: "destructive",
          });
        } finally {
  ...
  const createMarket = async (
    ...
    // Check sufficient balance for required stake + potential registration fee
    const stakeAmount = BET_CONFIG.CREATE_MARKET_STAKE;
    const registrationFeeWei = options?.registrationFeeWei ?? 0n;
    const bufferedRegistrationFee =
      registrationFeeWei > 0n ? (registrationFeeWei * 101n) / 100n : 0n;
    const totalValue = stakeAmount + bufferedRegistrationFee;
    ...
    const markClaimed = async () => {
      if (isSuccess && hash && address && marketIdRef.current !== null) {
        try {
          ...
          const bet = bets.find(
            (b: any) => b.marketId === marketIdRef.current?.toString(),
          );
...
export function useResolveMarket() {
  ...
  const resolveDataRef = useRef<{ marketId: string; result: boolean } | null>(
    null,
  );
    ...
    const persistResolution = async () => {
      if (isSuccess && hash && address && resolveDataRef.current) {
        try {
          ...
          await apiRequest(
            "PATCH",
            `/api/markets/${resolveDataRef.current.marketId}`,
            {
              status: "completed",
